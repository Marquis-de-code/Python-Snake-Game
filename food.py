...
class Food():
    ...
    def location_is_empty(self, location, snake):
        for segment in snake.body:
            if location.equals(segment) != False:
                return True
        return False

    def new_random_location(self, width, height, snake):
        point_isnt_valid = True
        while point_isnt_valid:
            proposed_location = Point2D(randint(0, width), randint(0, height))
            point_isnt_valid = self.location_is_empty(proposed_location, snake)
